...
#foreach( $association in $classObject.getSingleAssociations( ${includeComposites} ) )
	...
	#set( $inputName = "${roleName}InputId" )
		...
		$( "#multiSelectFor${roleName}" ).on("change", function(e)  
		{
			...
			if ( input.value != 'no selection' )
			{
				...
				var parentId = document.getElementById( '${pkExpression}' ).value;
		 		...
		 		var url 	= '/${className}/save${roleName}.action?${pkExpression}=' + parentId + "&childId=" + childId;
	 	...
	 	{			
	 		...
	 		var url 	= '/${classObject.getName()}/load.action?${pkExpression}=' + id;
		...
		if ( action == 'edit' )
		{
			...
			jQuery.ajax(
			{
		  		url: '/${classObject.getName()}/load.action?${pkExpression}=' + id,
		  		dataType: 'json',
			}).always(function( data ) 
			{
	...
	function deleteHelperFor${className}( nameOfRole, keyFieldName, ids )
	{
    	...
    	({
            ...
            callback: function(result) 
            {
				...
				var url 		= '/${classObject.getName()}/delete' + nameOfRole + '.action';
		    ...
		    <div class="sectionBodyClass">	  
	          <table class="formTableClass">
	            <tr class="formTRClass">
	              <td class="formTDClass">
<%
  String parentId	= "${pkExpression}=" + request.getParameter("${pkExpression}");
  String parentUrl	= "/${className}/save${roleName}?" + parentId;
  String addUrl 	= "/jsp/${associationClassType}ProfileForm.jsp?action=create&parentUrl=" + parentUrl;
  String deleteUrl 	= "/${className}/delete${roleName}?" + parentId;
  String modelUrl 	= "/${className}/load${roleName}?" + parentId;
%>	              
