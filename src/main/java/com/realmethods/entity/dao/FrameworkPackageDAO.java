...
public class FrameworkPackageDAO extends BaseDAO {
	...
	public List<FrameworkPackage> findAllFrameworkPackage(ScopeType scopeType, Long ownerId, String filter)
		...
		List<FrameworkPackage> list = null;
		...
		try
		{
			...
			if (query != null) {				
				if ( filter == null )
					...
					list = (List<FrameworkPackage>) query.list();
					// for now, we do not filter on anything based on a package attribute such as app type
					// but in the future may do something like the following
			...
			LOGGER.log(Level.SEVERE, msg, exc);
			throw new FrameworkDAOException(
					"FrameworkPackageDAO.findAllFrameworkPackage ", exc);
		} finally {
			closeSession();
		}
...
/*		
	...
	public List<FrameworkPackage> findAllFrameworkPackage()
		...
		StringBuilder buf = new StringBuilder(
				"from com.realmethods.entity.FrameworkPackage");
	...
	// Attributes
	private static final String FROM_QUERY_STRING = "from com.realmethods.entity.FrameworkPackage as field where ";
