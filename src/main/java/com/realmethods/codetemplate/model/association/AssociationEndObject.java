...
public class AssociationEndObject extends AppGenObject {
     ...
     * 
     * @return		 list of AttributeObjects
     */
     ...
     * 
     * @return		true/false on containing attributes
     */
    ...
    @Override
    /**
     * Evaluates if the input Object is equivalent to this instance.
     * 
     * @param 	o		Object to compare
     * @return			true/false on equivalence
     */
    ...
    /**
     * Helper method to create both the association and its sibling at the same
     * 
     * @param roleName			name of the role
     * @param type				type as ClassObject
     * @param multiplicity		ordinal of the association
     * @param navigableRoleName	role name of navigation end
     * @return
     */
    public static AssociationEndObject createBothEndsHelper( String roleName, String type, String multiplicity, String navigableRoleName ) {
		...
		associationEndObject.setRoleName( roleName);
		associationEndObject.setType(type);

		associationEndObject.setVisibility(visibility);
		associationEndObject.isNavigable(true);
		associationEndObject.isOrdered(isOrdered);
		associationEndObject.isAggregate(isAggregate);
		associationEndObject.isComposite(isComposite);
		associationEndObject.setLowerRange(1);
		associationEndObject.setUpperRange(1);
		...
		sibling.setUpperRange(1);

		// ------------------------------------------------------------
		//handling whether to make it navigable or not
		// ------------------------------------------------------------
		if ( navigableRoleName != null ) {
			sibling.setRoleName(navigableRoleName);
			sibling.isNavigable(true);
		} 
		else {
			sibling.setRoleName("unassigned");
    	...
    	else if ( multiplicity.equalsIgnoreCase("OneToMany") ) {
			associationEndObject.setLowerRange( 1 );
			associationEndObject.setUpperRange( -1 );
			sibling.setLowerRange( 1 );
			sibling.setUpperRange( 1 );				
    	}
    	else if ( multiplicity.equalsIgnoreCase("ManyToMany") ) {
			associationEndObject.setLowerRange( 1 );
			associationEndObject.setUpperRange( -1 );
			sibling.setLowerRange( 1 );
			sibling.setUpperRange( -1 );				
    	}
    	else if ( multiplicity.equalsIgnoreCase("ManyToOne") ) {
			associationEndObject.setLowerRange( 1 );
			associationEndObject.setUpperRange( 1 );
			sibling.setLowerRange( 1 );
			sibling.setUpperRange( -1 );
		...
		// ------------------------------------------------------------
    	// handle sibling end assigments
		// ------------------------------------------------------------
		associationEndObject.setSiblingAssociationEndObject(sibling);
		sibling.setSiblingAssociationEndObject(associationEndObject);

		final String msg = "AssociationEndObject is " + associationEndObject.toString() + " : " + multiplicity; 
		LOGGER.info( msg );
