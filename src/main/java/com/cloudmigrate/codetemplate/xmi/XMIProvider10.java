...
public class XMIProvider10 extends XMIProvider {
	...
	public int[] getLowerUpperRange(Node associationNode)
    {
        ...
        try
		{
	        ...
	        if(multiplicityNode != null)
	        {
	            ...
	            {
	                lowerRange = 1;
	                final String msg = " AssociationEnd " + getAssociationRoleName(associationNode) 
	                				+ " of " + getAssociationEndType(associationNode) 
	                				+ " has no lower range specified - assigning.";
	            ...
	            if(nUpper == null)
	            {
	                ...
	                if(multiplicityNode != null)
	                {
	                    multiplicityPath 	= getMultiplicityPath( multiplicityNode.getChildNodes().item(0) );
	                } else
	                {
	                    logDebugMessage("Null upper multiplicityNode " + multiplicityNode);
	                }
	            }
	            if(nUpper == null)
	            {
	                if(lowerRange <= 1)
	                    ...
	                    upperRange = -1;
	                LOGGER.log(Level.WARNING, "AssociationEnd " 
	                			+ getAssociationRoleName(associationNode) 
	                			+ " of " 
	                			+ getAssociationEndType(associationNode) 
	                			+ " has no upper range specified - assigning to " 
	                			+ String.valueOf(upperRange) + (upperRange != -1 ? "" : " (unlimited)"));
	            } else
	            if(nUpper.hasChildNodes())
	            {
	...
	public String getVisibility(Node methodNode) {
		...
		try {
			...
			final String msg = "Failure during locating the visibility indcator for method "
					+ methodName;
		...
		try {
			...
			while ((parameterNode = nl.nextNode()) != null) {
				nodeImpl = getXPathAPI().selectSingleNode(parameterNode,
						FOUNDATION_CORE_MODELELEMENT_NAME);
				if (nodeImpl != null)
					parameterName = nodeImpl.getChildNodes().item(0)
							.getNodeValue();
				else {
					LOGGER.log(Level.WARNING, "Incorrectly returned null parameter for method {0}", methodName);
					return (methodArgs);
				}
				
				msg = "Method " +  methodName + " has parameter name is " + parameterName;
				LOGGER.log(Level.INFO, msg);
				...
				if (nodeImpl != null)
					parameterType = getElementNameforID(nodeImpl.getNodeValue(),
							parameterNode);
				else {
					msg = "Incorrectly returned a null type for parameter " + parameterName + " on method " + methodName;
				...
				if (nodeImpl == null) {
					msg = "Incorrectly returned a null kind for parameter "
							+ parameterName + " on method " + methodName;
					LOGGER.warning(msg);
					return (methodArgs);
				}

				if (nodeImpl.getNodeValue().startsWith("in")) {
					if (parameterType.indexOf("void") == -1)
						methodArgs.getArgs().add(
								new MethodArg(parameterName, parameterType));
				} else {
	...
	public boolean getFinalIndicatorForAssociation(Node associationNode) {
		...
		String staticPath = getAssociationFinalIndicatorPath();
	...
	public boolean getVolatileIndicator(Node attributeNode) {
		...
		try {
			...
			final String msg = "Failure during locating the volatile indicator for attribute node "
					+ name;
			LOGGER.warning(msg);
		}
	...
	protected String getAbstractPath() {
		return "Foundation.Core.GeneralizableElement.isAbstract/@xmi.value";
	}
	...
	protected String getSuperTypeAsClassPath() {
		return "Foundation.Core.GeneralizableElement.generalization/Foundation.Core.Generalization";
	}

	protected String getSupertypeParentAsClassPath(Node childNode) {
		return "//*[@xmi.id='" + childNode.getNodeValue()
				+ "']/Foundation.Core.Generalization.parent/*/@xmi.idref";
	}

	protected String getSuperTypeAsInterfacePath() {
		return "Foundation.Core.ModelElement.clientDependency/Foundation.Core.Dependency";
	}
	...
	protected String getIDPath(String xmiID) {
		return "//*[@xmi.id='" + xmiID + "']/Foundation.Core.ModelElement.name";
	}

	protected String getAggregatorPath() {
		return "Foundation.Core.AssociationEnd.aggregation/@xmi.value";
	}

	protected String getCompositePath() {
		return "Foundation.Core.AssociationEnd.composite/@xmi.value";
	}

	protected String getNavigablePath() {
		return "Foundation.Core.AssociationEnd.isNavigable/@xmi.value";
	}

	protected String getOrderingPath() {
		return "Foundation.Core.AssociationEnd.ordering/@xmi.value";
	}

	protected String getVisibilityPath() {
		return "Foundation.Core.ModelElement.visibility/@xmi.value";
	}

	protected String getStaticPath() {
		return "Foundation.Core.Feature.ownerScope/@xmi.value";
	}
	...
	protected String getParameterPath() {
		return "*/Foundation.Core.Parameter";
	}

	protected String getParameterTypePath() {
		return "Foundation.Core.Parameter.type/*/@xmi.idref";
	}

	protected String getParameterKindPath() {
		return "Foundation.Core.Parameter.kind/@xmi.value";
	}

	protected String getAssociationFinalIndicatorPath() {
		return "Foundation.Core.AssociationEnd.changeability/@xmi.value";
	}

	protected String getAttributeFinalIndicatorPath() {
		return "Foundation.Core.StructuralFeature.changeability/@xmi.value";
	}
	...
	protected String getTaggedValueTaggedValuePath() {
		return "Foundation.Core.ModelElement.taggedValue/Foundation.Extension_Mechanisms.TaggedValue";
	}

	protected String getDefaultValuePath() {
		return "Foundation.Core.Attribute.initialValue/Foundation.Data_Types.Expression/Foundation.Data_Types.Expression.body";
	}
	...
	protected String getMultiplicityUpperPath() {
		return ".//Foundation.Data_Types.MultiplicityRange.upper";
	}
	...
	private static final Logger LOGGER = Logger
			.getLogger(XMIProvider10.class.getName());
