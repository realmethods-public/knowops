...
public class AttributeObject extends AppGenObject
{
	...
	public EnumClassObject getEnumClassObject() {
		...
		if (isFromEnumerator()) {
			...
			LOGGER.log(Level.WARNING, "AtributeObject.getEnumClassObject() - called but the type isn't of enum type. Type is "
							+ getType());
	...
	/**
	 * Assign the visbility field the provided argument.
	 * 
	 * @param v
	 */
	...
	protected void applyDefaultValue( String defValue )
	{
		if (isPrimaryKey)
		{
			LOGGER.log(Level.INFO, "AttributeObject.applyDefaulValue() - Attribute "
										+ getName()
